...
let app = new Vue ({
    ...
    computed : { //property: data that needs to change based on some other data
        ...
        /**
         * il filterScript è un array "aggiornato" in base all'inputContactSearch(cioè l'input utente),
         * dove tutto ciò che non corrisponde all'input utente (nella proprietà richiesta, in questo caso .name)
         * viene "filtrato via"
         * 
         * @returns prima di tutto il current array (this.contacts), a cui si applica il filtro che
         * a ciascun ciclo ritornerà il "valore" (input); 
         * l'arrow function viene performata ogni giro del ciclo per ogni item "valore" (input);
         * viene specificata la proprietà dentro cui controllare (attraverso match() ) l'input;
